...
struct FLeakyRectifyBackward {
	explicit FLeakyRectifyBackward(float a) : alpha(a) {}
	CNN_DEVICE_FUNC inline float operator()(float t, float d) const {
		return (t>0.0f) ? d : alpha*d;
	}
	float alpha;
};
...
struct FLeakyRectify {
	explicit FLeakyRectify(float a) : alpha(a) {}
	CNN_DEVICE_FUNC inline float operator()(float x) const {
