...
SimpleRNNBuilder::SimpleRNNBuilder(unsigned layers,
  ...
  model->push_scope(model->get_scope() + "/SimpleRNN/" + ns);
  char name_buf[128];
  for (unsigned i = 0; i < layers; ++i) {
	  sprintf(name_buf, "_%d", i);
	  Parameters* p_x2h = model->add_parameters({ (long)hidden_dim, (long)layer_input_dim }, (string)"x2h" + name_buf);
	  Parameters* p_h2h = model->add_parameters({ (long)hidden_dim, (long)hidden_dim }, (string)"h2h" + name_buf);
	  Parameters* p_hb = model->add_parameters({ (long)hidden_dim }, (string)"hb" + name_buf);
    vector<Parameters*> ps = {p_x2h, p_h2h, p_hb};
    if (lagging)
		ps.push_back(model->add_parameters({ (long)hidden_dim, (long)hidden_dim }, (string)"ps" + name_buf));
    params.push_back(ps);
