...
void LeakyRectify::forward(const vector<const Tensor*>& xs, Tensor& fx) const {
	assert(xs.size() == 1);
#if HAVE_CUDA
	assert(0);
#else
	auto x = **xs[0];
	*fx = x.unaryExpr(FLeakyRectify(alpha));
#endif
}

void LeakyRectify::backward(const vector<const Tensor*>& xs,
	const Tensor& fx,
	const Tensor& dEdf,
	unsigned i,
	Tensor& dEdxi) const {
	...
	assert(0);
#else
	*dEdxi += (*fx).binaryExpr(*dEdf, FLeakyRectifyBackward(alpha));
#endif
}
