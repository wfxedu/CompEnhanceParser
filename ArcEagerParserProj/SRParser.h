...
public:
	...
	LookupParameters* p_n; // pos tag embeddings

	Parameters* p_w2l; // word to LSTM input
	Parameters* p_n2l; // word to LSTM input
	...
	Expression build(unsigned word_id, unsigned pos_id, unsigned orgword_id, unsigned ner_id);
};
...
public:
	...
	Parameters* p_w2l; // word to LSTM input
	Parameters* p_p2l; // POS to LSTM input
	Parameters* p_tUNK; // pretrained word embeddings to LSTM input
	Parameters* p_ib; // LSTM input bias

	LSTMBuilder fw_lstm;
	LSTMBuilder bw_lstm;
	LSTMBuilder seg_lstm;
	//-------------
public:
	...
	vector<Expression> word2represent;
	vector<Expression> word2segment;
...
public:
	...
	void Init_state(const t_unvec& raw_sent, const t_unvec& sent, const t_unvec& sentPos, const t_unvec& sentNER, word_layer& layer);
public:
...
public:
	...
	Parameters* p_H2atten; 
	Parameters* p_H2atten_bias;


	Parameters* p_segBegin;
	Parameters* p_segEnd;
	Parameters* p_H4pre;
	Parameters* p_H4mid;
	Parameters* p_H4post;
	Parameters* p_Horg;
	Parameters* p_Dprg;
	Expression H4pre, H4mid, H4post;
	Expression Horg, Dorg, H2atten, H2atten_bias;
	...
	void Init_state(const t_unvec& raw_sent, 
		const t_unvec& sent, 
	...
	Expression build(Expression head,int headi, Expression dep,int depi, Expression relation);
