...
#include <unistd.h>
#endif
static vector<bool> s_IsPunt;
bool g_is_punt_en = false;
...
ParserTrainer::ParserTrainer(parser_config& cfg, DatasetTB& cpr, EmbedDict& edict) :
nn_config(cfg), sr_corpus(cpr), pretrained(edict)
{
...
void ParserTrainer::InitParser()
{
	{  // compute the singletons in the parser's training data
		...
		for (auto sent : sr_corpus.training.sentences1)
			for (auto word : sent.raw_sent) { 
				training_vocab.insert(word); counts[word]++; 
			}
		for (auto wc : counts)
	...
	if (g_is_punt_en) {
		s_IsPunt.resize(sr_corpus.pos2Int.nTokens + 1, false);
		string pos_punts[] = { "``","#","$","''",",","-LRB-","-RRB-",".",":","" };
		for (int i = 0;; i++) {
			if (pos_punts[i] == "")
				break;
			//int PU_POS = sr_corpus.pos2Int.tokens2Int[pos_punts[i]];
			//s_IsPunt[PU_POS] = true;
			int PU_POS = sr_corpus.pos2Int.get(pos_punts[i], -1);
			if (PU_POS >= 0)
				s_IsPunt[PU_POS] = true;
			else
				printf("Missing POS %s\n", pos_punts[i]);

			printf("\t info: PUNT %s-->%d\n", pos_punts[i].c_str(), PU_POS);
		}

	}
	else {
		s_IsPunt.resize(sr_corpus.pos2Int.nTokens + 1, false);
		int PU_POS = sr_corpus.pos2Int.get("PU", -1);
		if (PU_POS >= 0)
			s_IsPunt[PU_POS] = true;
		else
			printf("Missing POS %s\n", "PU");
		...
		if (logc % 25 == 1) { // report on dev set
			...
			for (unsigned sii = 0; sii < dev_size; ++sii) {
				...
				for (int iw = 0;iw < pred.size()-1;iw++) {
					if (s_IsPunt[sen_item.sentPos[iw]])// == PU_POS)
						continue;
			...
			if (correct_heads > best_correct_heads) {
				...
				char buff[512];
				fprintf(logO, "total=%.2f,\tepch=%.3f,\tdev_uas=%.5f,\tdev_uas1=%.5f -- save point %d \n",
					tot_seen, (float)(tot_seen / sr_corpus.training.sentences.size()),
					(float)(correct_heads / total_heads), (float)(label_right / total_heads), save_id);
				fflush(logO);

				sprintf(buff, "%s[s%d][e%.3f][U%.5f,L%.5f].bin", oname.c_str(), save_id, (float)(tot_seen / sr_corpus.training.sentences.size()),
					(float)(correct_heads / total_heads), (float)(label_right / total_heads));
				SaveModel(buff);
				save_id++;
