...
struct element_tr {
	...
	int parent;
	int prel_id;
	element_tr() : form_id(-1), form_useid(-1), pos_id(-1), parent(-1), prel_id(-1) {}
};
...
public:
	...
	static const char* NONE_NER;
	static const char* NONE_REL;


	void init(dictionary* w2i, dictionary* p2i, dictionary* r2i, dictionary* n2i,bool block) {
		words2Int = w2i; pos2Int = p2i; prel2Int = r2i; ner2Int=n2i;
		block_diction = block;
	}
	...
	void load(const char* path) {
		...
		int none_ner = ner2Int->reg(NONE_NER);
		...
		while (getline(tbFile, lineS)) {
			...
			if (block_diction) {
				...
				ele.ner_id = ner2Int->get(eles[4],-1);
